...
async function downloadReport() {
  ...
  if (csvString === "") {
    alert('Click on "Get report" first !');
    return;
  }
  const {
    data: { fileUrl, string },
  ...
  console.log(`file : ${fileUrl}, string: ${string}`);
...
function displayDownloadRow(download) {
  ...
  dateOfDownload.innerHTML = `${timeAndDate.getHours()}:${timeAndDate.getMinutes()}:${timeAndDate.getSeconds()}    ${timeAndDate.getDate()}-${
...
async function displayMonthlyReport(page) {
  ...
  expensesForSelectedMonth.forEach((expense) =>
    displaySingleRowExpense(expense)
  );

  csvString = createCsv(expensesForSelectedMonth);
}
